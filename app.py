...
def show_main_app():
    ...
    if st.session_state.current_path:
        ...
        for i, part in enumerate(path_parts):
            if part and part != s3_manager.app_prefix:
                ...
                if st.button(f"📁 {part}", key=f"path_{i}"):
    ...
    if all_items:
        ...
        if st.session_state.get('last_clicked_row') is not None:
            clicked_item = all_items[st.session_state.last_clicked_row]
            if clicked_item['type'] == 'folder':
                # Remove app_prefix from display path
                folder_path = clicked_item['path'].rstrip('/')
